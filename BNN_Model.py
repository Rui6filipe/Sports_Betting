...
class BayesianNN(nn.Module):
    ...
    def sample_elbo(self, input, target, loss_fn, sample_weights, kl_weight=0, num_samples=100):
        """
...
def train_test_split(df, n_ignore, n_test): 
    ...
    for season in seasons:
        ...
        season_indices = season_df.head(n_ignore).index  
...
def find_learning_rate(model, X_train, y_train, sample_weights, criterion, start_lr=1e-6, end_lr=1, num_iterations=1000):
    """ 
    Implementation of learning rate finder 
    """
    lrs = []
    losses = []
...
def setup_lr(X_training, y_train, features, initial_params):
    """
    Setup model and find maximum learning rate.
    """
